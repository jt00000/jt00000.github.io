I played RaRCTF as a member of team 3k and I solved 7 tasks(6 pwn, 1 rev). 
...
# not that simple
- The file name will be the flag. The directory list is obtained by gentdents (fd, buf, length) after opening the folder. (Rax = 78)
- The stack address is leaked, but I didn't use it because fixed addresses are more convenient.
...
To execute this binary, you have to prepare directory and dummy flag like this:
```
$ ls -lfR
.:
...
# boring-flag-checker
This is a Reverse task. The task described below was a handmade VM, so I solved it first. If you read the file and trace the relatively easy-to-read process, you will see that it consists of eight instructions. The cursor moves, there is an increase / decrease in the contents, there is a display, there is input, it looks like bf. There is spoiler in the Dockerfile, so there is no doubt.
```
$ head -n1 Dockerfile 
# bf-pwn
```
...
If you make prog.bin a bf code, you will get a lot of code. The place where `.` appears in the first half is probably the first message. The important processing is after that, and after receiving the input, it should be judged somewhere. The conditional branch of the bf code can only be expressed by whether the cell is 0 or not. If you stop before the subtraction process to check the difference at the end, you should be able to observe the difference with the correct flag.

If you stop at an appropriate processing break, enter `rarctf {aaaaaaaaaa}` and look inside the memory, the part of` rarctf {` will match, and the remaining difference will remain in the memory. So pick it up and restore the flag. This work was done this time with [elbrainfuck] (https://copy.sh/brainfuck/). It was easy to display the contents of the memory.
...
# boring-flag-runner
The instruction sequence has been checked in the previous task. We also know that there is no boundary check. The trouble is that the input command in the middle has a fixed offset and can be used only once. In this case, it is difficult to rely on leaks and re-entry, so we will attack with diff input. I subtracted to change `__libc_start_main ret` to one_gadget.
...
---
Below are the tasks I haven't solved. I challenged after the competition time.
...
When two of these are arranged side by side, the memory layout will be as follows. The `method` table, type is 16 bytes, followed by` dead`, `max_age`,` hunger`, `age`, and` name`, 1 byte each. All parameters are controllable.
...
Finally about the function `Sell`. The sale price is determined based on `age` and` max_age`. The maximum value is when `age` is half of` max_age`, so if you set the value so that `age + 1 = max_age`, you can see that the expected value of the sale price will be returned at the maximum. The `age` increment is random, so adjust it several times. You can just leave `hunger` and` dead` at 0.
...
The trouble is that the animals located in the upper chunk die at random timing, so check each command and repurchase if necessary.
...
# Return of Emoji DB
The source is also distributed. It seems to save emoji. It seems that the number of consecutive bits from msb of the first byte indicates the number of subsequent bytes.
...
After making an unsorted bin with free 8 times, point to it using oob writing. Arbitrary writes use a similar technique to create overlap chunks and manipulate tcache.
...
After calling the win function, the rest is trivial. I first leak the GOT function and then send ROP payload that pivots the stack to a fixed address and calls mprotect. Finally, send the shellcode (Search directory under `/pwn` & Open file that name is `*.txt`).
...
I enjoyed the task `boring-flag-runner`, which was relatively easy for its appearance, and `the mound`, which seemed easy and couldn't be solved easily.